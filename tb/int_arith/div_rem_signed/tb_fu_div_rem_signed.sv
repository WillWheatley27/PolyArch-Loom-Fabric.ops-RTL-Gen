// tb_fu_div_rem_signed.sv -- Self-checking testbench for fu_div_rem_signed
// (share group 2). Multi-cycle DUT: drive operands + op_sel, accept handshake,
// wait for out_valid, compare to an independent golden model (native signed
...
module tb_fu_div_rem_signed #(
  ...
  fu_div_rem_signed #(.WIDTH(WIDTH)) dut (
  ...
  // Two's-complement negate, WIDTH-relative.
  function automatic logic [WIDTH-1:0] negw(input logic [WIDTH-1:0] x);
    negw = (~x + WIDTH'(1));
  endfunction
  ...
  function automatic logic [WIDTH-1:0] golden(input logic [WIDTH-1:0] a,
    ...
    begin : golden_body
      ...
      if (b == {WIDTH{1'b0}}) begin : by_zero
        q = {WIDTH{1'b1}};   // RISC-V: quotient = -1
        r = a;               // RISC-V: remainder = dividend
      end : by_zero
      else if (a == {1'b1, {(WIDTH-1){1'b0}}} && b == {WIDTH{1'b1}}) begin : ovf
        // RISC-V: INT_MIN / -1 -> quotient = INT_MIN, remainder = 0.
        // (Native signed / overflows here, so encode the result explicitly.)
        q = a;
        r = {WIDTH{1'b0}};
      end : ovf
      else begin : normal
        q = sa / sb;         // signed, truncates toward zero
        r = sa % sb;         // sign follows dividend
      end : normal
      golden = sel ? r : q;
    end : golden_body
  endfunction
  ...
  logic [WIDTH-1:0] INT_MIN, INT_MAX, MINUS1;
  ...
  initial begin : main
    ...
    INT_MIN = {1'b1, {(WIDTH-1){1'b0}}};
    INT_MAX = {1'b0, {(WIDTH-1){1'b1}}};
    MINUS1  = {WIDTH{1'b1}};
    ...
    // Directed: normal division, all four sign quadrants (7/3, both taps).
    run_one(WIDTH'(7),  WIDTH'(3),  1'b0);  //  7 /  3 =  2
    run_one(WIDTH'(7),  WIDTH'(3),  1'b1);  //  7 %  3 =  1
    run_one(negw(WIDTH'(7)), WIDTH'(3), 1'b0); // -7 /  3 = -2
    run_one(negw(WIDTH'(7)), WIDTH'(3), 1'b1); // -7 %  3 = -1
    run_one(WIDTH'(7),  negw(WIDTH'(3)), 1'b0); //  7 / -3 = -2
    run_one(WIDTH'(7),  negw(WIDTH'(3)), 1'b1); //  7 % -3 =  1
    run_one(negw(WIDTH'(7)), negw(WIDTH'(3)), 1'b0); // -7 / -3 =  2
    run_one(negw(WIDTH'(7)), negw(WIDTH'(3)), 1'b1); // -7 % -3 = -1

    // |a| < |b|
    run_one(WIDTH'(5),  WIDTH'(10), 1'b0);  // 0
    run_one(WIDTH'(5),  WIDTH'(10), 1'b1);  // 5

    // Overflow corner: INT_MIN / -1
    run_one(INT_MIN, MINUS1, 1'b0);         // INT_MIN
    run_one(INT_MIN, MINUS1, 1'b1);         // 0
    run_one(INT_MIN, WIDTH'(1), 1'b0);      // INT_MIN
    run_one(INT_MIN, WIDTH'(1), 1'b1);      // 0
    run_one(INT_MAX, WIDTH'(1), 1'b0);      // INT_MAX
    run_one(INT_MAX, MINUS1,    1'b1);      // 0

    // Divide-by-zero (RISC-V): q = -1, r = dividend
    run_one(WIDTH'(0),  WIDTH'(0), 1'b0);   // -1
    run_one(WIDTH'(0),  WIDTH'(0), 1'b1);   //  0
    run_one(WIDTH'(5),  WIDTH'(0), 1'b0);   // -1
    run_one(WIDTH'(5),  WIDTH'(0), 1'b1);   //  5
    run_one(negw(WIDTH'(1)), WIDTH'(0), 1'b1); // dividend = -1
    run_one(INT_MIN, WIDTH'(0), 1'b1);      // dividend = INT_MIN
    ...
    if (error_count == 0) begin : pass_blk
      $display("PASS: fu_div_rem_signed WIDTH=%0d, %0d random vectors, 0 mismatches", WIDTH, NRAND);
    end : pass_blk
    else begin : fail_blk
      $display("FAIL: fu_div_rem_signed WIDTH=%0d, %0d mismatches", WIDTH, error_count);
      $fatal(1);
    end : fail_blk
...
endmodule : tb_fu_div_rem_signed
