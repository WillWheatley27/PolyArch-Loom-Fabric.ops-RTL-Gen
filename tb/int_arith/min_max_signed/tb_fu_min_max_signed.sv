// tb_fu_min_max_signed.sv -- Self-checking testbench for fu_min_max_signed
// (share group 6). Combinational DUT: drive operands + op_sel, settle, compare
// to an independent golden model (signed min/max via $signed). Directed corners
// (all sign quadrants, ties, INT_MIN/INT_MAX) + handshake corners + randomized.
// Parameterized by WIDTH (verilator -GWIDTH=8). Testbench only.

`timescale 1ns/1ps

module tb_fu_min_max_signed #(
  ...
  fu_min_max_signed #(.WIDTH(WIDTH)) dut (
  ...
  // Two's-complement negate, WIDTH-relative (for building signed test values).
  function automatic logic [WIDTH-1:0] negw(input logic [WIDTH-1:0] x);
    negw = (~x + WIDTH'(1));
  endfunction

  // Independent golden model (signed min/max).
  function automatic logic [WIDTH-1:0] golden(input logic [WIDTH-1:0] a,
    ...
    begin : golden_body
      ...
      // sel=0 -> minsi (smaller); sel=1 -> maxsi (larger)
  ...
  logic [WIDTH-1:0] INT_MIN, INT_MAX;
  ...
  initial begin : main
    ...
    // Sign quadrants (both taps): 5/3, -5/3, 3/-5, -5/-3
    check_vec(WIDTH'(5),        WIDTH'(3),        1'b0); // min = 3
    check_vec(WIDTH'(5),        WIDTH'(3),        1'b1); // max = 5
    check_vec(negw(WIDTH'(5)),  WIDTH'(3),        1'b0); // min = -5
    check_vec(negw(WIDTH'(5)),  WIDTH'(3),        1'b1); // max = 3
    check_vec(WIDTH'(3),        negw(WIDTH'(5)),  1'b0); // min = -5
    check_vec(WIDTH'(3),        negw(WIDTH'(5)),  1'b1); // max = 3
    check_vec(negw(WIDTH'(5)),  negw(WIDTH'(3)),  1'b0); // min = -5
    check_vec(negw(WIDTH'(5)),  negw(WIDTH'(3)),  1'b1); // max = -3

    // Ties
    check_vec(WIDTH'(7),        WIDTH'(7),        1'b0); // 7
    check_vec(WIDTH'(7),        WIDTH'(7),        1'b1); // 7
    check_vec(negw(WIDTH'(7)),  negw(WIDTH'(7)),  1'b0); // -7

    // Signed extremes
    check_vec(INT_MIN, INT_MAX, 1'b0); // min = INT_MIN
    check_vec(INT_MIN, INT_MAX, 1'b1); // max = INT_MAX
    check_vec(INT_MIN, WIDTH'(0), 1'b0); // INT_MIN
    check_vec(INT_MAX, negw(WIDTH'(1)), 1'b1); // max = INT_MAX (vs -1)
    check_vec(WIDTH'(0), WIDTH'(0), 1'b0); // 0

    // op_sel toggle on identical operands
    check_vec(negw(WIDTH'(9)), WIDTH'(4), 1'b0);
    check_vec(negw(WIDTH'(9)), WIDTH'(4), 1'b1);

    // Handshake corners
    check_backpressure(WIDTH'(5), WIDTH'(3), 1'b0);
    check_input_invalid();

    // Randomized (signed values, op_sel in {0,1})
    ...
    if (error_count == 0) begin : pass_blk
      $display("PASS: fu_min_max_signed WIDTH=%0d, %0d random vectors, 0 mismatches", WIDTH, NRAND);
    end : pass_blk
    else begin : fail_blk
      $display("FAIL: fu_min_max_signed WIDTH=%0d, %0d mismatches", WIDTH, error_count);
      $fatal(1);
    end : fail_blk
...
endmodule : tb_fu_min_max_signed
