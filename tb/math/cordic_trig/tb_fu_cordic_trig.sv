...
// Unary, latency-1 DUT (approximate sin/cos via CORDIC). TOLERANCE-based: the
// result is ~16-bit accurate, not bit-exact. Decode the DUT output to real and
// require |result - $sin/$cos(decode(x))| < TOL. Inputs in [-pi, pi]. Reports
// the worst-case error observed. WIDTH=32. TB only.
...
module tb_fu_cordic_trig #(
  ...
  localparam real TOL = 0.0005;   // ~2^-11 absolute

  fu_cordic_trig #(.WIDTH(WIDTH)) dut (
  ...
  // Encode a real to binary32 bits (for generating inputs; the oracle uses the
  // decoded value, so rounding here is harmless).
  function automatic logic [31:0] real_to_f32(input real v);
    ...
    begin
      ...
      if (av == 0.0) real_to_f32 = {sgn, 31'd0};
        ...
        sig = $rtoi((tmp - 1.0) * 8388608.0 + 0.5);   // round mantissa
        biased = ex + 127;
  ...
  initial begin : main
    ...
    // ---- directed angles, both sin and cos ----
    check_real(0.0, 1'b0); check_real(0.0, 1'b1);
    check_real(PI/2.0, 1'b0); check_real(PI/2.0, 1'b1);
    check_real(-PI/2.0, 1'b0); check_real(-PI/2.0, 1'b1);
    check_real(PI/4.0, 1'b0); check_real(PI/4.0, 1'b1);
    check_real(PI/6.0, 1'b0); check_real(PI/6.0, 1'b1);
    check_real(PI/3.0, 1'b0); check_real(PI/3.0, 1'b1);
    check_real(-PI/6.0, 1'b0); check_real(-PI/6.0, 1'b1);
    check_real(1.0, 1'b0); check_real(1.0, 1'b1);
    check_real(-1.0, 1'b0); check_real(-1.0, 1'b1);
    check_real(2.5, 1'b0); check_real(2.5, 1'b1);
    check_real(-2.5, 1'b0); check_real(-2.5, 1'b1);
    ...
    for (i = 0; i < NRAND; i++) begin : rand_loop
      t0 = $random; t1 = $random;
      r  = (real'($signed(t0)) / 2147483648.0) * PI;   // [-pi, pi)
      check(real_to_f32(r), t1[0]);
    end : rand_loop

    if (error_count == 0)
      $display("PASS: fu_cordic_trig WIDTH=%0d, %0d random vectors, max_err=%.3e (TOL=%.3e)",
               WIDTH, NRAND, max_err, TOL);
    else begin
      $display("FAIL: fu_cordic_trig WIDTH=%0d, %0d mismatches, max_err=%.3e", WIDTH, error_count, max_err);
      $fatal(1);
    end
    $finish;
  end : main
