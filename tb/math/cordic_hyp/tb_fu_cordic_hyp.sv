...
// Unary, latency-1 DUT (approximate sinh/cosh via hyperbolic CORDIC).
// PARAMETERIZED by (EXP_W, MANT_W): RTL generated per format (constants baked);
// compile the matching file with -GEXP_W/-GMANT_W. TOLERANCE: |dut -
// $sinh/$cosh(decode(x))| < TOL. Inputs in [-1.1, 1.1] (hyperbolic convergence).
// real_to_fp builds the mantissa bit-by-bit (NOT $rtoi). Worst-case abs error.
...
module tb_fu_cordic_hyp #(
  ...
  fu_cordic_hyp #(.EXP_W(EXP_W), .MANT_W(MANT_W)) dut (
  ...
  initial begin : main
    ...
    check(-0.75, 1'b0); check(-0.75, 1'b1);
    check(1.1, 1'b0); check(1.1, 1'b1);
    check(0.1, 1'b0); check(-1.0, 1'b1);

    // ---- randomized x in [-1.1, 1.1] ----
    for (i = 0; i < NRAND; i++) begin : rl
      t = $random;
      x = (real'($signed(t)) / 2147483648.0) * 1.1;
      check(x, t[0]);
    end : rl

    if (error_count == 0)
      $display("PASS: fu_cordic_hyp EXP_W=%0d MANT_W=%0d (WIDTH=%0d), %0d vectors, max_err=%.3e",
               EXP_W, MANT_W, WIDTH, NRAND, max_err);
    else begin
      $display("FAIL: fu_cordic_hyp EXP_W=%0d MANT_W=%0d, %0d mismatches, max_err=%.3e", EXP_W, MANT_W, error_count, max_err);
      $fatal(1);
    end
    $finish;
  end : main
